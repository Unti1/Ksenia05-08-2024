...
class Animal:
    ...
    def __init__(self,speed):
        self._cords = [10, 10, 10]
        self.speed = speed
    def move(self, dx, dy, dz):
        newz = self._cords[-1] *self.speed
        if newz<=0:
            print("It's too deep, i can't dive :(")
        else: 
            self._cords[0] = dx * self.speed
            self._cords[1] = dy * self.speed
            self._cords[-1] = dz * self.speed
            
    def get_cords(self):
        print(f'X:{self._cords[0]}, Y:{self._cords[1]}, Z:{self._cords[-1]}') 
    def attack(self):
        if self._DEGREE_OF_DANGER >=5:
            print("Be careful, i'm attacking you 0_0")
        else: print("Sorry, i'm peaceful :)")    
...
class Bird(Animal):
    ...
    def lay_eggs(self):
        ...
        print(f"Here is {x} eggs for you")
